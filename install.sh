...
SRV_PUBKEY=""
SRV_DRY_RUN=false
# Флаги базовой линии (§3.48). Приехали в harden-host.sh волной 1 и были
# доступны ТОЛЬКО при его прямом запуске — то есть почти никому: и панель, и
# управляемый сервер получают харднинг через ЭТОТ файл, а не руками.
#
# Уезжают они ОКРУЖЕНИЕМ (FD_OWNER_IP/FD_NO_SWAP), а не argv. Причина: между
# install.sh и harden-host.sh стоит посредник — prepare-host.sh в режиме
# панели и bootstrap-server.sh в режиме сервера, — и argv-контракт пришлось бы
# держать согласованным в трёх файлах сразу. Переменные harden-host.sh читает и
# так (секция разбора аргументов), это его собственный, уже существующий вход.
OWNER_IPS=()
NO_SWAP=false
SRV_ADMIN_KEYS=()
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    # Накопительный, как и у harden-host.sh: адресов у владельца может быть
    # несколько (дом, офис, VPN). Проверку формы делает harden-host.sh —
    # `valid_ignoreip` там БЕЛЫЙ список форм, и вторая его копия здесь
    # неизбежно разъехалась бы с первой. Цена: опечатка в адресе роняет
    # установку на шаге харднинга, а не на разборе аргументов. Установщик
    # идемпотентен — повторный запуск с исправленным адресом безопасен.
    --owner-ip)
      [[ $# -ge 2 ]] || log_err "--owner-ip требует значение"
      OWNER_IPS+=("$2")
      shift 2
      ;;
    --no-swap) NO_SWAP=true; shift ;;
    # Проброс в bootstrap-server.sh (§1.4). Без него флаг был бы доступен
    # только при прямом запуске bootstrap'а — а прямого запуска у него больше
    # нет: с BKP-F27 единственный публичный путь подготовки сервера идёт через
    # ЭТОТ файл. Валидацию делает bootstrap: белый список типов ключа там один,
    # и вторая его копия здесь разъехалась бы с первой (тот же довод, что у
    # --owner-ip выше).
    --admin-key)
      [[ $# -ge 2 ]] || log_err "--admin-key требует значение"
      SRV_ADMIN_KEYS+=("$2")
      shift 2
      ;;
...
Флаги базовой линии безопасности (работают в ОБОИХ режимах, уезжают в
harden-host.sh):
  --owner-ip <адрес|подсеть>
                          — не банить этот адрес в fail2ban. Повторяемый.
                            По умолчанию НЕ задан: у домашнего провайдера
                            адрес динамический, и протухшая запись означает,
                            что перебор с чужого теперь адреса перестанут
                            банить.
  --no-swap               — не заводить файл подкачки на малопамятном хосте
                            (по умолчанию скрипт заводит его, если своей
                            подкачки нет вовсе и на диске есть запас).

Только режим --server:
  --admin-key '<ssh-ed25519 …>'
                          — положить ЛИЧНЫЙ ключ владельца в
                            /root/.ssh/authorized_keys. Повторяемый.
                            Подготовка отключает вход по паролю, а ключ,
                            который она кладёт, — панельный (restrict,pty,
                            приватная половина в базе панели): владельцу он
                            не даёт ничего. Без этого флага твой путь входа
                            после прогона — только консоль хостера.
USAGE
      exit 0
      ;;
...
# Кадр окружения для harden-host.sh. Экспортируем ТОЛЬКО то, что владелец
# попросил явно: пустой `FD_OWNER_IP=` был бы неотличим от «не задано», а
# `FD_NO_SWAP=` (пустая строка) harden-host.sh как раз и трактует как «не
# задано» — но полагаться на это молча не стоит, поэтому переменных просто нет,
# когда флага не было.
if [[ ${#OWNER_IPS[@]} -gt 0 ]]; then
  export FD_OWNER_IP="${OWNER_IPS[*]}"
  log_info "fail2ban не будет банить: ${FD_OWNER_IP}"
fi
if [[ "${NO_SWAP}" == "true" ]]; then
  export FD_NO_SWAP=1
  log_info "файл подкачки заводиться НЕ будет (--no-swap)"
fi
...
# ─── 2. Check OS ─────────────────────────────────────────────────────────────
#
# ПОРЯДОК: проверка ОС стоит ДО первого `apt-get` (§1.6). Пока она была ниже
# установки `jq`, владелец RHEL, Fedora или Arch получал не матрицу
# поддерживаемых систем, а `apt-get: command not found` — то есть ответ про
# пакетный менеджер на вопрос про операционную систему. И, что хуже, на такой
# машине `apt-get update` мог и найтись (например, через прослойку), успеть
# что-то сделать и оставить след — до того, как выяснилось бы, что установщик
# тут вообще не работает. Проверка, которая ничего не меняет, обязана идти
# перед всем, что меняет.
...
# ─── 2b. jq — для разбора ответов Releases API ───────────────────────────────
#
# Стоит ПОСЛЕ проверки ОС намеренно (§1.6): это первая команда установщика,
# которая что-то меняет на хосте, и до неё все проверки обязаны пройти.
#
...
if [[ "${MODE}" == "server" ]]; then
  ...
  # Явная проверка длины, а не `${arr[0]+…}`-идиома: ключ содержит пробелы, и
  # любая форма без кавычек вокруг раскрытия разорвала бы его на слова.
  if [[ ${#SRV_ADMIN_KEYS[@]} -gt 0 ]]; then
    for k in "${SRV_ADMIN_KEYS[@]}"; do
      BOOT_ARGS+=(--admin-key "${k}")
    done
  fi
  bash "${BOOT}" "${BOOT_ARGS[@]}"
  exit 0
...
echo ""
# ─── Превью сайтов: НЕ ставится сама, и это решение ─────────────────────────
# Браузер весит 280 МБ. Класть его в тарбол значит платить эту цену на КАЖДОМ
# обновлении парка ради необязательной картинки в карточке проекта, поэтому он
# ставится отдельной командой и только если владелец её захотел (ROADMAP §3.12,
# решение владельца 2026-09-04). Пока команда не выполнена, панель просто не
# снимает превью — это штатное состояние, а не недоустановка.
echo -e " ${BOLD}Превью сайтов (необязательно):${NC}"
echo "    sudo fd-screenshot-setup --install    # +280 МБ: браузер для скриншотов"
echo ""
