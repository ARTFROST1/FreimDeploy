...
| Доступ к GitHub | Готовить заранее НЕ нужно: панель предложит установить GitHub App первым экраном после настройки. Токен (fine-grained, `Contents: Read`) остался запасным путём на том же экране |
...
Поля для GitHub-токена в мастере НЕТ (с 2026-09-06 шага три: пароль, домен,
готово). Не проси у человека токен на этом экране. Подключение GitHub —
первый экран панели уже на её домене: кнопка «Установить GitHub App», а под
ней «Не получается? Ввести токен вручную».
